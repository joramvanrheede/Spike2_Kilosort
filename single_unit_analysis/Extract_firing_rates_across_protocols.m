...
for i = 1:length(matched_ChR2)
    ...
    % (This lumps all units together)
    
    if ~isempty(ChR2_thalamus_spikes)
        ChR2_thalamus_baseline_rate         = spike_rate_by_trial(ChR2_thalamus_spikes,baseline_win);
        ChR2_thalamus_target_rate           = spike_rate_by_trial(ChR2_thalamus_spikes,target_win);
        ChR2_thalamus_delta_rate            = mean(ChR2_thalamus_target_rate - ChR2_thalamus_baseline_rate);
        
        [ChR2_thalamus_p, ChR2_thalamus_h]  	= ranksum(ChR2_thalamus_baseline_rate,ChR2_thalamus_target_rate);
    else
        ChR2_thalamus_delta_rate    = NaN;
        ChR2_thalamus_p             = NaN;
        ChR2_thalamus_h             = NaN;
    end
    
    if ~isempty(ArchT_thalamus_spikes)
        ArchT_thalamus_baseline_rate        = spike_rate_by_trial(ArchT_thalamus_spikes,baseline_win);
        ArchT_thalamus_target_rate          = spike_rate_by_trial(ArchT_thalamus_spikes,target_win);
        ArchT_thalamus_delta_rate         	= mean(ArchT_thalamus_target_rate - ArchT_thalamus_baseline_rate);
        
        [ArchT_thalamus_p, ArchT_thalamus_h]    = ranksum(ArchT_thalamus_baseline_rate,ArchT_thalamus_target_rate);
    else
        ArchT_thalamus_delta_rate    = NaN;
        ArchT_thalamus_p             = NaN;
        ArchT_thalamus_h             = NaN;
    end
    
    if ~isempty(ChR2_ArchT_thalamus_spikes)
        ChR2_ArchT_thalamus_baseline_rate   = spike_rate_by_trial(ChR2_ArchT_thalamus_spikes,baseline_win);
        ChR2_ArchT_thalamus_target_rate  	= spike_rate_by_trial(ChR2_ArchT_thalamus_spikes,target_win);
        ChR2_ArchT_thalamus_delta_rate     	= mean(ChR2_ArchT_thalamus_target_rate - ChR2_ArchT_thalamus_baseline_rate);
        
        [ChR2_ArchT_thalamus_p, ChR2_ArchT_thalamus_h]  = ranksum(ChR2_ArchT_thalamus_baseline_rate,ChR2_ArchT_thalamus_target_rate);
    else
        ChR2_ArchT_thalamus_delta_rate    = NaN;
        ChR2_ArchT_thalamus_p             = NaN;
        ChR2_ArchT_thalamus_h             = NaN;
    end
    ...
    this_results_table  = table(unique_unit_ids(:), unit_depths, ...
                                ChR2_thalamus_delta_rate, ChR2_thalamus_p, ChR2_baseline_rates, ChR2_target_rates, ChR2_trial_rates, ChR2_delta_rates, ChR2_p_vals, ChR2_h_up_down, ...
                                ArchT_thalamus_delta_rate, ArchT_thalamus_p, ArchT_baseline_rates, ArchT_target_rates, ArchT_trial_rates, ArchT_delta_rates, ArchT_p_vals, ArchT_h_up_down, ...
                              	...
                              	'VariableNames',{'unit_id', 'unit_depth', ...
                               	'ChR2_thal_delta', 'ChR2_thal_p','ChR2_baseline','ChR2_target','ChR2_trial','ChR2_delta','ChR2_p','ChR2_h_up_down', ...
